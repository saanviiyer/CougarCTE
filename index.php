...
<script type="text/javascript" src="//translate.google.com/translate_a/element.js?cb=googleTranslateElementInit"></script>
  ...
  <ul class="navbar-links">
    ...
    <h1 style="color: white">Cougar CTE
  </h1>
...
while ($row = mysqli_fetch_assoc($result)) {
    $classname = htmlspecialchars($row['name']);  // Prevent XSS by escaping HTML special characters
    $cluster = strtolower($row['cluster']); // Simplify the cluster name handling
    $buttonid++;
...
$buttonid = 0;
while ($row = mysqli_fetch_assoc($result)) {
    $organizationname = htmlspecialchars($row['name']);  // Prevent XSS by escaping HTML special characters
    $buttonid++;
    echo '<li class="filterDiv"><a href="organizationPage.php?button=' . $buttonid . '">' . $organizationname . '</a></li>';
}
mysqli_stmt_close($stmt);
mysqli_close($connection);
?>
